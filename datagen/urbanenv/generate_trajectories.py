...
def write_turtle_xml(outpath: Path, all_trajectories: Dict[int, List[Tuple[float, float, float]]],
                     speeds: Dict[int, float], gen_params: dict):
    """Write all trajectories to a TurtleMobility XML file."""
    root = ET.Element("movements")

    # Add generation parameters as comment
    comment = ET.Comment(f' Generated by: {gen_params["_generator"]} ')
    root.insert(0, comment)
    params_comment = ET.Comment(f' Parameters: {json.dumps(gen_params["_params"])} ')
    root.insert(1, params_comment)
...
"""
    ...
    gen_params = {
        "_generator": "generate_trajectories.py",
        "_params": {
    ...
    write_turtle_xml(outpath, all_trajectories, speeds, gen_params)
