...
usage() {
  cat <<'EOF'
Usage:
  ./datagen/run_spoofing_aware_trajectory_planning_batch.sh [scenario selection] [options]
...
# ---- Auto-added by datagen/run_spoofing_aware_trajectory_planning_batch.sh ----
[Config {aware}]
extends = {scenario}
seed-set = {seed}
...
if [[ "$DO_PLOT" == "1" ]]; then
  ...
  echo "== Generating 3D trajectory overlay charts =="
  if ! ./scripts/docker-run.sh python3 datagen/plot_sweep_3d_trajectories.py \
    --generated-dir "$GEN_DIR" \
    --out-dir "$RUN_ROOT/charts" \
    --sample-every 3; then
    PIPELINE_FAILED=1
    echo "[WARN] 3D trajectory overlay chart generation failed."
  fi
fi
